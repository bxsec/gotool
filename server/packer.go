...
func (d *DefaultPacker) Pack(entry *protocol.Message) ([]byte, error) {
	dataSize := len(entry.Payload)
	...
	copy(buffer[8:], entry.Payload)              // write data
	return buffer, nil
}
...
func (d *DefaultPacker) Unpack(reader io.Reader) (*protocol.Message, error) {
