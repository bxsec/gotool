...
// MaxRpcMessageLength is the max length of a RpcMessage.
...
var (
	...
	// ErrRpcMessageTooLong RpcMessage is too long
	ErrRpcMessageTooLong = errors.New("RpcMessage is too long")
...
// RpcMessageType is RpcMessage type of requests and responses.
type RpcMessageType byte

const (
	// Request is RpcMessage type of request
	Request RpcMessageType = iota
	// Response is RpcMessage type of response
	Response
)
...
// RpcMessage is the generic type of Request and Response.
...
// NewRpcMessage creates an empty RpcMessage.
func NewRpcMessage() *RpcMessage {
...
// Header is the first part of RpcMessage and has fixed size.
// Format:
//
type Header [12]byte
...
// RpcMessageType returns the RpcMessage type.
func (h Header) MessageType() RpcMessageType {
	return RpcMessageType(h[2]&0x80) >> 7
}

// SetRpcMessageType sets RpcMessage type.
func (h *Header) SetMessageType(mt RpcMessageType) {
	h[2] = h[2] | (byte(mt) << 7)
}

// IsHeartbeat returns whether the RpcMessage is heartbeat RpcMessage.
...
// IsOneway returns whether the RpcMessage is one-way RpcMessage.
...
// RpcMessageStatusType returns the RpcMessage status type.
func (h Header) MessageStatusType() RpcMessageStatusType {
	return RpcMessageStatusType(h[2] & 0x03)
}

// SetRpcMessageStatusType sets RpcMessage status type.
...
// Clone clones from an RpcMessage.
...
func (m RpcMessage) EncodeSlicePointer() *[]byte {
...
// WriteTo writes RpcMessage to writers.
func (m RpcMessage) WriteTo(w io.Writer) (int64, error) {
...
func (m *RpcMessage)GetHeaderLen() int {
	return 16
}

func (m *RpcMessage)ParseToBodyLen(r io.Reader) (int,error) {
...
// Read reads a RpcMessage from r.
func Read(r io.Reader) (*RpcMessage, error) {
	msg := NewRpcMessage()
	err := msg.Decode(r)
	if err != nil {
		return nil, err
...
// Decode decodes a RpcMessage from reader.
func (m *RpcMessage) Decode(r io.Reader) error {
	// validate rest length for each step?
...
// Reset clean data of this RpcMessage but keep allocated data
